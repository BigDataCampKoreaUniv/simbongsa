...
def register_users():
    ...
    if request.method == 'POST':
        if register_user_form.validate_on_submit():
            ...
            major = ""
            univer = ""
            return redirect(url_for('/'))  # register가 완성되면 초기페이지로
        else:
...
#    user_id = 3   # temporary
    ...
    # 로그인 안된것도 처리를 하면 된다!
    similar_list = maxuser(user_id, user_list,vol_list)
...
#    category_names, category_receivers, category_tags = classification(vol_content[similar_list[i][0]][8:-5])
    ...
    for i in range(len(similar_list)):
        likes_bool.append(already_liked(user_id,similar_list[i][1]))
        cur.execute("select * from volunteer where id=" + str(similar_list[i][1]))
        row = cur.fetchone()
        category_names.append(max_classification(row[8:-5]))
        similar_star_rate.append(similar_star(similar_list[i][0]))

    return render_template('index-google-map.html', category_name=category_names, check_flag=likes_bool,
